  ...
  <v-dialog :model-value="show" fullscreen @update:modelValue="$emit('close')">
    <v-card>
      ...
      <v-card class="mx-5 mt-3">
        <v-card-title class="text-h5 bold">Comments</v-card-title>
        <v-card-text>
          <!-- Add New Comment -->
          ...
          >
            <!-- Comment Header -->
            <div
              class="comment-header d-flex align-center justify-space-between"
            >
              ...
              <div class="comment-actions d-flex align-center">
                <v-btn icon @click="upvote(comment.id)"> 👍 </v-btn
                >{{ comment.upvotes }}
                <v-btn icon @click="downvote(comment.id)"> 👎 </v-btn
                >{{ comment.downvotes }}
                <v-btn icon @click="bookmark(comment.id)"> 🔖 </v-btn>
                <v-btn icon @click="share(comment.id)"> 📤 </v-btn>
              </div>
            </div>
